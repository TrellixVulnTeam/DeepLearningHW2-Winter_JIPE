...
def mlp_experiment(
    ...
    res = trainer.test_epoch(dl_test=dl_test)
...
def cnn_experiment(
    ...
    model_temp = model_cls(ds_train[0][0].shape, 10, channels=channels,
                           ...
                           conv_params=dict(kernel_size=2, stride=1, padding=6),
                           activation_type='relu', pooling_type='avg', pooling_params=dict(kernel_size=1))
    ...
    optimizer = torch.optim.SGD(params=model.parameters(), nesterov=True, **oprim_dict)
    loss_fn = torch.nn.CrossEntropyLoss()
